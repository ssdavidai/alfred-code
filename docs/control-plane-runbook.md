...
Only after diagnostics and a read-only cycle are clean should `apply = true` be set. With a configured Project, every open issue is projected into the agent-silent Backlog; the legacy `auto_intake` and intake-label settings apply only when no Project scheduler is configured. Order candidates in Inbox, drag selected cards into Sprint Queue, and use the dashboard Start Sprint action or `alfred-code sprint-start`. Only active sprint cards move through Specifying and post plans. Nothing builds until the exact GitHub approval command is present. For isolated sprint delivery, set `sprint_integration_enabled = true`, `sprint_auto_merge = true`, and a non-empty `sprint_verify_command` only after branch protection and CI are healthy.

A 21-point plan closes the active sprint item into Needs Splitting without creating children. Open that card in the local dashboard to inspect the planner's evidence and proposed boundaries. The nonce-bound Split action is the sole authorization to create one native GitHub sub-issue per proposed job. The parent remains open as the epic, every child is placed in Inbox, and no child is specified or built until it is deliberately selected for a later sprint. If GitHub or Project synchronization fails partway through, use Resume splitting; durable markers adopt prior effects instead of duplicating issues.
...
Use a GitHub Project board grouped by Control stage as the draggable product surface. Ordinary Backlog and Inbox cards are agent-silent. Sprint Queue is an explicit commitment waiting for Start Sprint. The dashboard links the sprint integration worktree, current branch, retrospective state, and final integration PR. Worker PRs are reviewed and merged into the sprint branch automatically; they do not close issues. Use Slack as an inbox only.

In integration mode, the last lane merge does not close the sprint. The controller opens one sprint delivery PR to `main`, waits for repository CI, runs one independent read-only exact-SHA retrospective over all committed issues and their combined diff, and reruns `sprint_verify_command`. Only a passing retrospective for the unchanged head permits the guarded final merge; the controller then closes the issues and sprint. A blocked or rejected item, red CI, merge conflict, incomplete acceptance evidence, changed SHA, or failed retrospective keeps the sprint open and is shown on the dashboard. Repair the evidence or add bounded corrective work to the same sprint; never bypass the gate merely to clear the board.
...
If a reviewer fails, the controller sends the exact findings to a scoped worker in the original workspace and repairs the same PR without expanding the approved paths. The old SHA-bound verdict cannot approve the repair commit; green CI causes a new independent review workspace. The repair handoff is bound to the failed SHA and a controller nonce, and automatic attempts stop at `review_repair_max_attempts` (default four). If that cap is exhausted, inspect the preserved PR and workspace; do not raise the cap or bypass scope merely to force progress. A manual in-scope commit to the same PR also changes HEAD and triggers a fresh review.

If a local reviewer launch marker says `running` but its recorded wrapper process no longer exists, the controller revalidates the read-only workspace and restarts that exact-SHA review once. A second vanished process at the same SHA blocks for operator attention instead of looping. Explicit `failed` or `exited` markers still block immediately with their recorded reason.
