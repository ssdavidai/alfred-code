...
Use a GitHub Project board grouped by Control stage as the draggable product surface. Ordinary Backlog and Inbox cards are agent-silent. Sprint Queue is an explicit commitment waiting for Start Sprint. Open the linked Superset workspace when you want to see an agent's terminal. Use the PR for code, CI, smoke evidence, and the final merge decision. Use Slack as an inbox only.

When the last active item becomes Done or terminally blocked, the sprint closes automatically. Done cards remain historical sprint membership. Each blocked card moves to the top of Inbox, never to Sprint Queue or a new sprint. The controller non-destructively closes only PRs it can prove it owns, supersedes the failed graph and approval, and spends one planner attempt producing a fresh Inbox plan. You may comment, approve, or reject that plan while refining the backlog; approval still cannot create a job or launch an agent. To execute it, drag the card into Sprint Queue and start a later sprint. If Inbox planning fails, the controller records the error and stops retrying until you deliberately select the work again.
