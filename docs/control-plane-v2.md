...
GitHub is the product-management and decision surface. With a configured Project scheduler, every open issue is projected into an agent-silent draggable Backlog; the legacy auto-intake and label gate apply only without that scheduler. The operator moves ordered candidates to Inbox, selects Sprint Queue, and explicitly starts the sprint. Only then does the controller inspect live repository and GitHub state, generate a lane-aware plan pinned to the current delivery-branch SHA, attach an evidence-based Fibonacci estimate and cross-issue dependencies, validate it deterministically, and comment it on the issue. The operator approves exactly that plan with `/approve-plan <full-plan-sha256>`, rejects it with `/reject-plan <full-plan-sha256>`, or leaves specification feedback as a normal comment. Superset creates and owns the isolated workspaces and starts the configured worker/reviewer runtime. GitHub PRs and checks are the delivery authority. Slack is optional notification output only.

The controller never gives merge authority to an agent. With sprint integration disabled it remains human-merge-only. With `sprint_integration_enabled` and `sprint_auto_merge`, it may squash an exact CI-green reviewed lane head only into the recorded sprint branch, and may merge that sprint branch to protected `main` only after final CI and an exact-SHA retrospective both pass. Every merge is bound with `--match-head-commit` and never uses `--admin`. It may close an open PR only when an automatic re-plan has proved the PR belongs to the exact superseded controller job; that reversible retirement keeps the branch, commits, worktree, and audit history intact. It never deletes a branch, force-pushes, or resets a branch. Workspace deletion is separately configured, is off by default, and is eligible only after GitHub reports the associated PR as merged.
...
| Did the operator approve? | exact GitHub comment authored by `ssdavidai` | `approvals` |
...
Before review, the controller fetches the complete PR file list from GitHub and compares every filename to the approved plan paths. An extra file blocks regardless of worker claims or hooks. Green CI is not enough unless the PR body also contains its smoke-evidence section. The reviewer manifest carries those approved paths only as read-only package-root hints so the launcher can expose validated offline dependency caches; reviewer profiles and hooks still permit writes only to the review result marker.

Review passes only when GitHub CI is green and the immutable trusted GitHub operator `ssdavidai` posts `<!-- alfred-code-review:<exact-head-sha>:pass -->` after the controller requested review. A marker posted before the independent review intent, by another actor, or for another SHA is ignored. A new commit changes HEAD, invalidates the old marker automatically, and produces a distinct review workspace. A passing review makes the job `ready_merge`. In human-merge mode the operator merges in GitHub. In sprint auto-merge mode the controller first verifies that the PR base is the recorded sprint branch, then squash-merges the exact reviewed head without bypassing protection. The issue becomes `integrated`, not closed.

A failing review marker starts a bounded repair cycle in the original worker workspace. The controller preserves the approved plan and lane, proves that the worktree is clean at the exact failed PR SHA, writes a one-attempt `.lane` handoff, and launches only the scoped worker preset. The repair agent remains offline, cannot write Git metadata, and can edit only the already-approved paths. Its result must echo the exact failed SHA, attempt number, and controller nonce; stale result files cannot complete the handoff. The trusted controller then rejects deletions and scope drift, reruns verification, commits and pushes the repair, and waits for CI plus a new independent review workspace at the new SHA. `review_repair_max_attempts` defaults to four; exhausting it leaves the PR blocked for operator attention instead of looping forever. Red CI still blocks separately.

## Issue lifecycle

Product stage and execution state are separate durable facts. `backlog`, `inbox`, and `sprint_queue` are operator-owned. Ordinary cards in those stages are agent-silent. Starting a sprint snapshots card order and commitment, changes selected cards to `active`, and permits `planning`. In integration mode it also records one remote sprint branch and one Superset integration worktree. `awaiting_approval` means a current immutable plan exists with no valid decision. A normal comment authored by `ssdavidai` after that plan invalidates it and supplies context to a fresh plan. An exact rejection from `ssdavidai` is terminal for that sprint and materializes no jobs. Every other account's comment remains untrusted data and cannot drive a transition or enter planner context. `building` means at least one approved job is active, including an exact-SHA repair. `ready_merge` means every job is either ready or merged. `integrated` means all current jobs are present in the sprint branch but the issue remains open. `completed` means the final sprint delivery is on `main` and the issue is closed. An integration-mode sprint closes only after every item is integrated, the combined PR's CI is green, the read-only exact-SHA retrospective and trusted verification pass, and the guarded final merge is observed. Any failed or missing evidence leaves it open.

A terminally blocked item is returned to the top of Inbox, never enrolled in another sprint. Before that transition, every open PR in its unfinished graph must be proven to belong to the exact controller job; otherwise sprint closure fails closed. Owned PRs are closed with an audit comment while branches, commits, workspaces, reviews, and blocker evidence remain intact. The old graph is superseded, its approval is revoked, lane leases are released, and a durable carryover flag requests exactly one fresh planning attempt in Inbox. This is the only agent-active Inbox state. The replacement can be reviewed and approved there, but the controller's execution gate forbids job materialization and Superset launch until the operator moves the card to Sprint Queue and explicitly starts a future sprint. Moving the card elsewhere, approving or rejecting the replacement, or a failed planning attempt clears the carryover flag, so Inbox cannot create a perpetual planner loop.

GitHub Projects is a guarded two-way contract. In Backlog, Inbox, Sprint Queue, and Needs Splitting, the operator's card move is authoritative. Once a sprint item enters specification, the controller owns its execution stage. Every poll refreshes Project position so the manual card order becomes the scheduler rank. The controller updates `Control stage`, native `Sprint`, `Story points`, `Risk`, `Plan hash`, `Lane set`, and `Runtime`; SQLite preserves sprint membership, commitment versus mid-sprint additions, estimate evidence, rollover, and immutable events. If project sync fails, execution freezes passive-stage adoption but already-authorized work can continue from durable state. Slack notification failure never mutates delivery truth.
...
`apply = false` is the shipped default. In this mode a cycle reads GitHub and updates observations but does not invoke a planner, comment, create a branch, create a workspace, or start an agent. `doctor` must validate the repo, lane policy, database, GitHub credentials authenticated specifically as `ssdavidai`, Superset authentication, and optionally the GitHub Project before `apply` should be enabled.
