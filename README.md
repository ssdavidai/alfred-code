...
GitHub Backlog -> ordered Inbox -> explicit Sprint Queue commitment
             ...
             -> lane PR + CI -> exact-HEAD independent review
             -> guarded squash into isolated sprint branch
             -> final sprint PR + CI + exact-SHA retro -> guarded merge to main
```

No agent can start without a current validated plan and exact approval authored by the immutable trusted GitHub operator `ssdavidai`. Comments from every other account are retained only as untrusted GitHub data: they cannot approve, reject, change planning context, suppress controller markers, or satisfy independent review. Configuration cannot widen this boundary, and the controller refuses to write comments or merge unless `gh` is authenticated as `ssdavidai`. When sprint auto-merge is enabled, the controller may squash only an exact reviewed head into that sprint's recorded integration branch, then merge the final exact retrospective-approved sprint head into protected `main`; it never uses `--admin`. It does not reset, force-push, delete branches, expose secrets, or write outside approved scope. Workspace cleanup is disabled by default.
...
With `sprint_integration_enabled`, starting a sprint creates one durable Superset integration worktree and remote sprint branch. Every plan is pinned to that branch's observed Git object. Worker PRs target the sprint branch, never `main`; a CI-green exact-SHA-reviewed lane PR is squash-merged automatically only when `sprint_auto_merge` is also enabled. Completed issues remain open and `integrated` while the sprint continues. After every committed item is integrated, the controller opens one sprint delivery PR to `main`, waits for full repository CI, launches one read-only exact-SHA retrospective over the combined diff, reruns `sprint_verify_command`, and merges without an admin bypass only if both checks pass. It then closes the issues and sprint. Any missing item, conflicting or red final PR, failed retrospective, SHA drift, or wrong base keeps the sprint open with the failure visible on the dashboard.

When integration mode is disabled, sprint membership still remains immutable history and the earlier human-merge/carryover behavior applies. Done work stays Done. A terminally blocked item is moved to the top of Inbox, not into another sprint; its old approval and unfinished jobs are superseded, and any open PR must be proven controller-owned before the controller closes it non-destructively. The Inbox carryover flag causes exactly one new planning attempt. The fresh plan may be reviewed, revised, approved, or rejected in Inbox, but even an approved plan cannot create jobs or agents until the operator explicitly moves the issue to Sprint Queue and starts a future sprint. A planning failure clears the carryover flag instead of creating an unbounded token loop.
