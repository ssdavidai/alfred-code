...
4. **Integration smoke against `staging.alfred.black`** — the designated scratch tenant (cx33, IP 167.233.24.182), a faithful clone of home (data/creds/onboarding synced; comms channels blank). For a UI lane needing its backend live, or a cross-lane flow: deploy your branch(es) to staging and curl/exercise the real thing. SSH: `ssh -i ~/.ssh/alfred-black-verify -o IdentityAgent=none -o BatchMode=yes -o StrictHostKeyChecking=no root@staging.alfred.black` → `/opt/alfred`. Pull your branch, `docker compose up -d <service>`, exercise `https://staging.alfred.black/...`. Throwaway tenant — safe to mutate, NEVER on home.
   - **Live click-through UI verification** (the real cure for #214/#217 smoke-theater) is done by **interactive Claude on Sir's Mac-mini Desktop driving the real Chrome** that's logged into staging as Alfred — NOT by you (a headless lane agent has no GUI), and NOT via Playwright/headless browsers (Sir's explicit preference). So if your UI lane can't be functionally proven by curl/local-run, say so honestly and tag it **"needs a live staging UI pass"** — that pass happens in an interactive session, not here. "Needs live UI pass" is an acceptable honest-partial; faking a UI smoke is not.
5. **Post-merge integration** on home is the final gate AFTER merge+deploy. If a lane genuinely can't be fully exercised pre-merge even on staging, say so and name the post-merge live-test — but you must STILL do #1 (local) first. "Needs post-merge verification" is acceptable; "I only read the code" is not.
