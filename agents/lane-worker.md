...
4. **Integration smoke against `staging.alfred.black`** — the designated scratch tenant (cx33, IP 167.233.24.182, built 2026-05-31). For a UI lane that needs its backend lane live, OR a cross-lane flow you can't fully exercise locally: deploy your branch(es) to staging and click/curl the real thing. SSH: `ssh -i ~/.ssh/alfred-black-verify -o IdentityAgent=none -o BatchMode=yes -o StrictHostKeyChecking=no root@staging.alfred.black` → `/opt/alfred` (clone of ssdavidai/alfred). Pull your branch there, `docker compose up -d <service>`, then exercise `https://staging.alfred.black/...`. This is a throwaway tenant — safe to mutate, NEVER do this on home. Integration secrets (Composio/AgentMail/Groq/voice) are stubbed there, so flows needing those are degraded — note it. Core (web/ctrl-api/hermes/alfred-learn/temporal/vault) is real.
